...
def get_tags_suggestions(api_key, image_url):
    ...
    response = requests.post('https://api.clarifai.com/v2/models/bd367be194cf45149e75f01d59f77ba7/outputs', headers=headers, data=json.dumps(data))
    tags=[]
    output=response.json()['outputs'][0]['data']['concepts']
    for x in range(len(output)):
        tags.append(output[x]['name'])
    return tags
